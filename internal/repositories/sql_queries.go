...
const (
	...
	orderAccrualGetOrdersByStatusQuery = "SELECT * from order_accrual where status in ($1)"
	orderAccrualCreateQuery            = "insert into order_accrual ( user_id, order_id, status, amount) values (:user_id, :order_id, :status, :amount)"
	orderAccrualUpdateQuery            = "update  order_accrual set amount = :amount, status = :status where order_id = :order_id"

	orderWithdrawalCreateQuery            = "insert into order_withdrawal ( user_id, order_id, amount) values (:user_id, :order_id, :amount)"
